defmodule Beamcore.Agent.FilesystemJournal do
  ...
  def internal_relative_paths,
    do: [".beamcore/snapshots", ".beamcore/recovery", ".beamcore/memory"]
  ...
  def changes_since(workspace_root, start_position)
    ...
    mutations =
      ...
      |> Enum.filter(fn mutation ->
        integer(Map.get(mutation, "position"), 0) > start_position
      end)
  ...
  defp directory_file_count(entries) when is_map(entries) do
    ...
    |> Enum.reduce(0, fn
      ...
      %{"type" => "directory", "entries" => sub_entries}, acc ->
        acc + directory_file_count(sub_entries)
  ...
  defp ensure_not_internal(relative_path) do
    if internal_path?(relative_path),
      do: {:error, "BeamCore internal safety-journal and memory paths are not agent-writable"},
      else: :ok
  end
  ...
  defp journal_path(workspace_root),
    do: Path.join([snapshot_store_root(workspace_root), @journal_file])

  defp blob_dir(workspace_root),
    do: Path.join([snapshot_store_root(workspace_root), @blob_dir])

  defp blob_path(workspace_root, hash), do: Path.join(blob_dir(workspace_root), hash)

  defp recovery_base(workspace_root, restore_id),
    do: Path.join([workspace_store_root(workspace_root), @recovery_dir, restore_id])

  defp restore_intent_path(workspace_root, restore_id),
    do: Path.join([snapshot_store_root(workspace_root), "restores", "#{restore_id}.json"])

  defp safety_revision_path(workspace_root, restore_id),
    do: Path.join([snapshot_store_root(workspace_root), "restores", "#{restore_id}.safety.json"])

  defp snapshot_store_root(workspace_root),
    do: Path.join([workspace_store_root(workspace_root), @snapshot_dir])

  defp workspace_store_root(workspace_root) do
    workspace_id =
      workspace_root
      |> Path.expand()
    ...
    Path.join([System.user_home!(), ".beamcore", "workspaces", workspace_id])
  end
