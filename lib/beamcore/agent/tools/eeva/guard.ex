defmodule Beamcore.Agent.Tools.Eeva.Guard do
  @moduledoc false
  ...
  @type runtime :: %{caps: map(), workspace_root: binary()}

  def prepare(quoted, caps) when is_map(caps) do
    with :ok <- validate(quoted, caps) do
  ...
  defp validate(quoted, caps) do
    {_quoted, errors} =
      Macro.prewalk(quoted, [], fn node, errors ->
        {node, validate_node(node, caps, errors)}
      end)

    case errors |> Enum.reverse() |> Enum.uniq() do
      [] -> :ok
      messages -> {:error, "Guard violation: " <> Enum.join(messages, " ")}
    end
  end

  defp validate_node({form, meta, args}, _caps, errors)
  ...
  defp validate_node({:__aliases__, meta, parts}, _caps, errors) when is_list(parts) do
    module = Module.concat(parts)

    if module in @blocked_model_runtime_modules do
      [
        "References to #{inspect(module)} are unavailable inside Eeva at line #{line(meta)}."
        | errors
      ]
    else
      errors
  ...
  defp validate_node(
    ...
    if module in [File, Path, System, Beamcore.Memory] do
      [
        "Capturing #{inspect(module)}.#{function}/#{arity} would bypass Eeva runtime instrumentation at line #{line(meta)}. Call it directly instead."
        | errors
      ]
    else
      errors
  ...
  defp validate_node({{:., meta, [module_ast, function]}, call_meta, args}, caps, errors)
    ...
    cond do
      ...
      is_nil(module) and not (args == [] and Keyword.get(call_meta, :no_parens, false)) ->
        [
          "Dynamic module invocation is unavailable because it would bypass Eeva runtime checks."
          | errors
        ]
  ...
  defp validate_node(_node, _caps, errors), do: errors
    ...
    Macro.prewalk(quoted, fn
      ...
      when is_atom(function) and is_list(args) ->
        case module_from_ast(module_ast) do
          File -> guard_call(:file, [function, args], dot_meta, call_meta)
          Path when function == :wildcard -> guard_call(:wildcard, args, dot_meta, call_meta)
          System when function == :cmd -> guard_call(:system_cmd, args, dot_meta, call_meta)
          Beamcore.Memory -> guard_call(:memory, [function, args], dot_meta, call_meta)
          _ -> node
        end
  ...
  defp guard_call(function, args, dot_meta, call_meta) do
  ...
  defp call_file([path | _rest] = args, :ls) do
    case apply(File, :ls, args) do
      {:ok, entries} -> {:ok, filter_internal_entries(path, entries)}
      other -> other
    end
  end

  defp call_file([path | _rest] = args, :ls!) do
    entries = apply(File, :ls!, args)
    filter_internal_entries(path, entries)
  end

  defp call_file(args, function), do: apply(File, function, args)

  defp filter_internal_entries(_path, entries) when is_list(entries) do
    Enum.reject(entries, &internal_workspace_entry?/1)
  end

  defp filter_internal_entries(_path, entries), do: entries

  defp internal_workspace_entry?(entry) when is_binary(entry) do
    String.downcase(entry) in [".beamcore", "beamcore.snapshot", "beamcore.recovery"]
  end

  defp internal_workspace_entry?(_entry), do: false
  ...
  defp call_tracked_file(args, function, runtime) when function in [:write, :write!] do
    ...
    if successful_file_result?(result) do
      ...
      journal_or_raise(
        FilesystemJournal.record_file_write(path, before_state, after_bytes, tool: "eeva")
      )
    end
  ...
  defp call_tracked_file(args, function, runtime) when function in [:rename, :rename!] do
    ...
    if successful_file_result?(result) do
      ...
      journal_or_raise(
        FilesystemJournal.record_file_write(target, target_before, after_bytes, tool: "eeva")
      )
    end
  ...
  defp call_tracked_file(args, function, runtime)
       when function in [:touch, :touch!, :chmod, :chmod!] do
    [path | _rest] = args
    before_state = snapshot_before!(path, runtime)
    result = apply(File, function, args)
  ...
  defp call_tracked_file(args, function, runtime) when function in [:ln_s, :ln_s!] do
    ...
    if successful_file_result?(result) do
      journal_or_raise(
        FilesystemJournal.record_file_write(target, target_before, "", tool: "eeva")
      )
    end
  ...
  defp authorize_runtime_path(_relative, _mode, _caps), do: :ok

  defp authorize_memory_write(runtime) do
    cond do
      Map.get(runtime.caps, :allow_memory_write, true) ->
        :ok

      true ->
        {:error, "Memory mutation is unavailable in this mode."}
    end
  end
  ...
  defp runtime! do
    case Process.get(@process_key) do
      %{caps: caps, workspace_root: workspace_root} ->
        %{caps: caps, workspace_root: workspace_root}

      _ ->
        raise "Eeva runtime runtime was not installed in the execution process."
    end
  end
  ...
  defp direct_internal_guard_call?(module), do: module == __MODULE__
