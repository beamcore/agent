defmodule Beamcore.Agent.Tools.Python do
        ...
        parameters: %{
          ...
          properties: %{
            command: %{
              ...
              description: "The Python project management command to run (e.g. 'test', 'lint', 'venv')"
            },
            args: %{
              type: "string",
            ...
            venv: %{
              type: "string",
              description: "Virtual environment name or path to use. If not specified, uses project's default venv or system Python.",
              default: ""
            }
          },
          required: ["command"]
  ...
  defp create_venv(venv_name) do
    ...
    case runner().(python_executable(), ["-m", "venv", venv_path], stderr_to_stdout: true, env: []) do
      {output, 0} ->
        %{
          "ok" => true,
          "exit_code" => 0,
      ...
      {output, exit_code} ->
        %{
          ...
          "stdout" => output,
          "stderr" => "",
          "summary" => "Failed to create virtual environment '#{venv_name}': #{String.trim(output)}"
        }
    end
  end
