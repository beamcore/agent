defmodule Beamcore.Agent.Tools.WebGet do
        ...
        parameters: %{
          ...
          properties: %{
            ...
            headers: %{
              ...
              description:
                "Dictionary of HTTP headers (e.g., {\"Accept\": \"text/html\"})."
            },
            timeout: %{
              type: "integer",
  ...
  def execute(params) do
    ...
    if String.starts_with?(url, "http://") || String.starts_with?(url, "https://") do
      ...
      normalized_defaults = Map.new(@default_headers, fn {k, v} -> {String.downcase(k), {k, v}} end)
  ...
  defp make_request(url, headers, timeout) do
    ...
    case http_client().request(:get, request, http_opts, []) do
      {:ok, {{_version, status, _reason}, resp_headers, resp_body}} ->
        ...
        metadata_str =
          build_metadata(status, raw_size, final_size, formatted_headers, is_cleaned, was_truncated)
  ...
  defp html_content?(nil, body) do
    ...
    String.starts_with?(trimmed, "<html") || String.starts_with?(trimmed, "<!doctype html") || String.starts_with?(trimmed, "<")
  end
  ...
  defp strip_noise_tags(html) do
    noise_tags = [
      "script", "style", "svg", "iframe", "noscript", "canvas", "nav",
      "footer", "aside", "form", "button", "select", "option", "dialog",
      "map", "area", "track"
    ]
  ...
  defp collapse_table_whitespace(html) do
    pattern = ~r/(<\/?(?:table|thead|tbody|tr|td|th)\b[^>]*>)\s+(?=(?:<\/?(?:table|thead|tbody|tr|td|th)\b[^>]*>))/is
