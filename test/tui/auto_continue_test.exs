defmodule Beamcore.TUI.AutoContinueTest do
  ...
  test "does not auto-continue if screen_type is not :research", %{state: state} do
    state = %{state | screen_type: :agent, session: %{state.session | screen_type: :agent, messages: [%{role: "assistant", content: "hello"}]}}
    updated = Events.maybe_auto_continue(state)
    assert updated == state
  end

  test "does not auto-continue if status is :paused", %{state: state} do
    state = %{state | status: :paused, session: %{state.session | messages: [%{role: "assistant", content: "hello"}]}}
    updated = Events.maybe_auto_continue(state)
    assert updated == state
  end
  ...
  test "does not auto-continue if last message contains RESEARCH_COMPLETE", %{state: state} do
    state = %{state | session: %{state.session | messages: [%{role: "assistant", content: "Done. RESEARCH_COMPLETE"}]}}
    updated = Events.maybe_auto_continue(state)
    assert updated == state
  end

  test "auto-continues if last message is assistant response without RESEARCH_COMPLETE", %{state: state} do
    state = %{state | session: %{state.session | messages: [%{role: "assistant", content: "Planning complete. Proceeding to gather data."}]}}
  ...
  test "API.extract_reasoning extracts reasoning from <think> tags in content" do
    ...
    assert Beamcore.Agent.Chat.API.extract_reasoning(msg) == {"Here is the answer.", "I should calculate X."}
  end
  ...
  test "Dispatcher normalizes write_file and read_file tool calls" do
    {name1, args1} = Beamcore.Agent.Tools.Dispatcher.normalize_tool_call("write_file", %{"path" => "a.md", "content" => "test"})
    assert name1 == "modify_file"
    assert args1 == %{"operation" => "create_file", "path" => "a.md", "content" => "test", "overwrite" => true}

    {name2, args2} = Beamcore.Agent.Tools.Dispatcher.normalize_tool_call("write_to_file", %{"path" => "b.md", "content" => "test2"})
    assert name2 == "modify_file"
    assert args2 == %{"operation" => "create_file", "path" => "b.md", "content" => "test2", "overwrite" => true}

    {name3, args3} = Beamcore.Agent.Tools.Dispatcher.normalize_tool_call("read_file", %{"path" => "c.md"})
    assert name3 == "read"
