defmodule Beamcore.Agent.Chat.CorrectionCatchTest do
  ...
  describe "CorrectionCatch.correct_and_rollover/3" do
    ...
    test "chat loop detects loop, self-corrects, and resumes execution seamlessly" do
      ...
      updated_session = Beamcore.Agent.Chat.Loop.send_message(session, "step 5", nil, nil, silent: true)

      # Verify the sequence of calls and messages
      assert_receive {:completion_called, 0, _} # Main assistant call
      assert_receive {:completion_called, 1, _} # Correction call
      assert_receive {:completion_called, 2, _} # Resumed execution call
      ...
      assert (system_msg["content"] || system_msg[:content]) =~ "Diagnosis: Getting stuck using 'actually'"
    end
  end
end
