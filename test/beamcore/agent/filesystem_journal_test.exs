defmodule Beamcore.Agent.FilesystemJournalTest do
  ...
  test "overlapping human edit becomes conflict and current file is preserved", %{
    ...
    assert Path.wildcard(Path.join(recovery_root(root), "*/current/conflict.txt")) != []
  end
  ...
  test "corrupted blob is detected and not restored", %{root: root, session: session} do
    ...
    blob =
      ...
      |> Path.join("*")
      |> Path.wildcard()
      |> Enum.find(&(File.read!(&1) == "original\n"))
  ...
  test "applying restore intent recovers from safety revision on restart", %{root: root} do
    ...
    restore_dir = restore_root(root)
    File.mkdir_p!(restore_dir)
    File.mkdir_p!(blob_root(root))

    File.write!(Path.join(root, "restore.txt"), "partially restored\n")

    bytes = "pre-restore\n"
    hash = sha256(bytes)
    File.write!(Path.join(blob_root(root), hash), bytes)
  ...
  test "checkpoint journal boundary is exact", %{root: root, session: session} do
    ...
    base_position = FilesystemJournal.journal_position(root)
    checkpoint_a = checkpoint!(session, "A")
    ...
    assert checkpoint_a.filesystem_revision["journal_position"] == base_position
    assert checkpoint_b.filesystem_revision["journal_position"] == base_position + 1
  ...
  test "snapshot store is blocked from agent tools", %{session: session} do
    ...
    refute result["ok"]
    assert result["summary"] =~ "internal safety-journal"
  end
  ...
  defp blob_root(root), do: Path.join([workspace_store_root(root), "snapshots", "blobs"])
  defp recovery_root(root), do: Path.join([workspace_store_root(root), "recovery"])
  defp restore_root(root), do: Path.join([workspace_store_root(root), "snapshots", "restores"])

  defp workspace_store_root(root) do
    workspace_id =
      root
      |> Path.expand()
